/**
 * 剧本快捷键：Ctrl+B/J/M + 数字 → 模拟"听到这一句" → 交给理解链路
 *
 * ── 它在链路里的位置 ──────────────────────────────────────────────
 ...
 * 2026-09-17 要的"1–10 用 B、11–20 用 J、21–25 用 M"）；`Ctrl+Q+L` 是建单
...
export type ScriptShortcutEntry = {
  /**
   * 触发键的**复合 id**：`"b:1"` / `"j:0"` / `"m:5"`。
   *
   * 为什么不是裸数字：同一个数字键在三段里含义不同（`Ctrl+B+1` 是第 1 条、
   * `Ctrl+J+1` 是第 11 条），裸数字会让条目表、查表与清单三处都失去唯一性。
...
export function useScriptShortcut({
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      /* 只对确实属于本序列的按键拦默认行为：
         · armed —— Ctrl+B/J/M 在部分浏览器有默认动作（B 是加粗等），
           必须拦，否则序列还没按完页面就动了；
