...
test("键位顺序 = 剧本出场顺序（键盘行序：1..0 → q 那一排 → a、s）", () => {
  /*
    表里用的是**文档条号 + 圈号标签**，而剧本用的是圈号；键位顺序则是键盘行序。
    三份顺序必须同时对齐：这里不重打映射，而是用"键在该键表里的下标"与"条目在表中的
    出场顺序"交叉验证 —— 顺序错位会让"第 N 个键"落到别的轮次上。
  */
...
test("⑬ 轮（无触发语的那轮）用该段台词兜底，且指向 ⑬", () => {
  /*
    ...
    当"听到的话"，并指到 ⑬。否则 `Ctrl+M+E` 会像早期实现那样按了毫无反应。
