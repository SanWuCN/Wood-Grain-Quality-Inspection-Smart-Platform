...
test("稿子里各轮的上一句原话，必须命中对应轮次", () => {
  const cases: [string, string][] = [
    ["小木，读取这份工单，整理任务范围和出发清单。", "①"],
    ["小木，请你联网查找一下现场情况，提供近三个月的天气数据，评估该地古建可能存在的风险。", "②"],
    ["小木，核对开工清单，显示还没完成的项目。", "④"],
    ["小木，读取校验结果和现有标定表，给出本次补偿参数建议。", "⑤"],
    ["小木，持续检查地图和视频通道，只提醒影响作业的异常。", "⑥"],
    ["小木，检查这批重建素材，列出缺失文件和需要重看的画面。", "⑦"],
    ["小木，请比较这四组木构件，按可见异常给出优先复核顺序。", "⑧"],
    ["小木，打开你标记的原图，把疑点区域放大。", "⑨"],
    ["小木，先检查巡检任务的设备、地图和作业区，生成下发预览。", "⑩"],
    ["小木，请汇总本次异常证据，并生成补充数据和模型适配的任务清单。", "⑫"],
    ["小木，把补采、数据审核和适配验证拆成任务卡，关联本次异常批次。", "⑬"],
    ["小木，对照采样计划检查接收清单，缺什么就列什么。", "⑭"],
    ["小木，启动数据清洗，列出需要人工审核的记录，生成数据集划分。", "⑮"],
    ["小木，汇总新旧模型的验证结果，检查部署条件。", "⑯"],
    ["小木，调用本批次分析流程，完成图像标注和雷达分析，再按测区融合结果。", "⑱"],
    ["小木，把图像疑点和同测区响应放在一起，列出需要补核的项目。", "⑲"],
    ["小木，结合本次检测记录，生成工单草稿，列出复核位置、处理建议和附件。", "⑳"],
    ["小木，生成本次任务复盘，重点写异常原因、处置过程和后续待办。", "㉑"],
    ["小木，根据校验结果整理交付摘要，打开还需要处理的条目。", "㉒"],
  ];
  for (const [utterance, expected] of cases) {
    const m = matchScriptRound(stripWakeWord(utterance));
    assert.ok(m, `「${utterance}」没有返回匹配`);
    assert.equal(m.round.roundNo, expected, `「${utterance}」命中了 ${m.round.roundNo} 而不是 ${expected}：${m.reason}`);
    assert.equal(m.verdict, "hit", `「${utterance}」判定为 ${m.verdict}：${m.reason}`);
  }
});
