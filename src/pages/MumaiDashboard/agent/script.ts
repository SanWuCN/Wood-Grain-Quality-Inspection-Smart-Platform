 ...
 * 一轮脚本交互
 *
 * ⚠ 本文件**不做值导入**（只有下面这一条 `import type`）：`tools/验收-*` 与审计脚本
 *   直接用 Node 的原生类型剥离加载它，值导入会把整条依赖链拖进来。
 */
import type { NavOp } from "./navOp";
...
export type ScriptRound = {
  ...
  nav?: {
    ...
    batch?: string;
    /**
     * 带进页面的**检索问题**（知识库检索验证页会直接执行它）。
     *
     * ⚠ 为什么需要它（用户 2026-10-01：「第一个对话跳转不对，跳那啥都没有评委看什么」）：
     *   第①轮的反问句是「过去三个月到过多少地方巡检、多少风险点、修复多少」，
     *   落点是知识库的**检索验证**页 —— 而那一页默认只有一个空输入框 + 三条示例，
     *   跳过去屏幕上什么都没有。带上 `q` 之后，页面打开就把这一问**跑出来**，
     *   评委看到的是真实的检索问题、命中的资料与原文定位。
     */
    q?: string;
    /**
     * 跳转**之后**在页面里做的那一件事（词表见 `agent/navOp.ts`）。
     *
     * 有几轮的主角不是"某一页"，而是"页面上要真的发生一件事"——
     * 这时只跳转是不够的（页面默认状态里什么都没有）：
     *   · ⑱「打开归档版本的验证摘要」→ `archive-verify`：把交付文件校验**真的跑一遍**；
     *   · ㉑「调用本批次分析流程」→ 待补（词表里加一项即可）。
     * 由 `navigate_page` 在导航之后派发，目标页面用 `useNavOp(op, handler)` 领取。
     */
    op?: NavOp;
  };
};
...
export const SCRIPT_ROUNDS: ScriptRound[] = [
  {
    ...
    /*
      ...
      ⚠ `q` 是必须的（用户 2026-10-01：「第一个对话跳转不对，跳那啥都没有评委看什么」）：
        只跳到「检索验证」页签的话，屏幕上是一个空输入框 + 三条示例 —— 评委什么也看不到。
        这句话的检索问题随 URL 一起带过去，页面打开就把它跑出来（问题 + 命中资料 + 原文定位）。

      ⚠ **问句要"像小木刚说的那句"、又要真能命中**（用户 2026-10-01 第二次口径：
        「现在小木第一句对话的效果呈现不行」）。原来的 `q` 是「风险点处置记录」——
        三个名词堆在一起，投屏上跟小木刚念的整句问话对不上，像是另一个人随手搜的。
        但直接把整句问话（「查过去三个月我们一共到过多少个地方巡检…」）塞进去，
        2–4 元 TF-IDF 会被长句稀释到阈值以下 → **0 命中**，页面反过来显示"未检索到匹配证据"。
        所以取一个折中：带上时间范围与两个关键名词，读起来就是这句问话的主干。
        实测（2026-10-01，服务版本 KB-033）：
          · 「近三个月巡检风险点与处置记录」→ 5 条命中，最高 0.32（月台 · 九月巡检报告）
          · 「风险点处置记录」（旧）→ 5 条，最高 0.51，但读起来是关键词不是问句
          · 「近三个月巡检地点与风险点处置记录」→ 0 条（多一个"地点"就掉到阈值以下）
    */
    nav: {
      route: "/knowledge",
      tab: "search",
    ...
    lines: [
      {
         ...
         *
         * ⚠ 数字**不再出现在台词里**（2026-09-23 复核）：旧句曾要求含
         *   「26.4、78%、1.6、CFG-02」，但用户改写后的三段式台词一个数字都没有，
         *   而 `scriptLines.test.ts` 核对的是「参数对照表 / 标定配置 / 保留待核验 / 复核后下发」
         *   这四个语义片段 —— 具体读数由 `envPreset.ts` 的录入值（22 ℃ / 58 %RH / 0.6 m/s /
         *   101 kPa）与页面上的参数对照表给出，台词只负责说清"做了什么、谁来复核"。
  ...
  },
  {
    ...
    */
    nav: {
      /*
        ── 2026-10-01 改落点（用户口径：「针对一些只有跳转不太合适的对话加上特殊页面或操作」）──
        原来落在 `/firmware?tab=training`（训练验证）——可这一轮讲的是**归档版本**的验证摘要，
        训练页上根本没有那张摘要，观众只看到"跳了一页 + 弹一个浮层"。
        归档验证摘要的真实位置是**报告归档页**：交付清单 10 组 24 项 + 校验结果
        （一致 / 缺失 / 摘要不一致 + 逐项 SHA-256）。那一页默认写着「尚未运行校验」，
        所以光跳过去仍然什么都没有 —— 配一个 `op: "archive-verify"`，
        跳过去之后**把真实的交付文件校验跑一遍**，摘要当场出现。
      */
      route: "/archive",
      op: "archive-verify",
    },
