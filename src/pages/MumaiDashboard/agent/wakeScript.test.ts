...
test("模糊输入（错字 / 漏字 / 词序乱）仍走剧本", () => {
  const cases: [string, string][] = [
    ...
    /*
      ...
      （见 22c2b6f：triggers 改为短关键词，便于演示前准备），
      该轮（2026-09-17 重排后是 **⑤ 天气风险查询**）现登记
      「近三个月天气 / 天气数据 / 古建风险 / 现场情况」；
...
test("只说「工单」不得触发第②轮（交接文档明令）", () => {
  /*
    ...
    背景：接单整理（2026-09-17 重排前是 ①，重排后是 **②**）的 triggers 里
    曾经有一条 2 字短语「工单」，已删除；
...
test("§9.2 的歧义已消除：「打开工单」不再命中 ③（也不该命中 ②）", () => {
  /*
    ...
    修之前的实测：`打开工单` 命中 ④（0.75，**旧编号**，即现在的 ③）——
    因为「开工清单」与「打开工单」的字符多重集高度重合（开/工/单 都在）。
    后果是用户想"打开这张工单看看"，平台却播了开工清单那一轮的台词并跳了过去。

    修法（本次一起做的两件事）：
      · 接单整理的触发补成含动作的完整说法（「读取这份工单」等），
        开工清单核对用「核对开工清单」，两者字面不再混淆；
...
test("剧本每一轮都能被它自己的第一个触发说法命中（数据自洽）", () => {
  for (const round of SCRIPT_ROUNDS) {
    /*
      ⚠ ⑬ 由**本地事件**触发（triggerSource: "local-event"，2026-09-17 重排前是 ⑪），
      按设计没有语音触发短语，所以这里跳过它 —— 不是放宽要求，而是它的触发来源
      本来就不是语音。
    */
