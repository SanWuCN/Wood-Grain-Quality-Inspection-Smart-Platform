 ...
 *
 * ── 为什么单独一个纯函数 ────────────────────────────────────────────
 * `Ctrl+B/J/M + 数字` 是**两段式序列**（先按段前缀，再按目标键），判定里全是
 ...
 *   · 按住 Ctrl，先按**段前缀**（B / J / M），再在 1.5 秒内按数字；
 ...
 * 随后又改了一次：「ctrl加n换成加j的，ctrl加n有功能冲突了」
 * —— **Ctrl+N 是浏览器默认的「新建窗口」**，按下去页面会开出新窗口，所以第二段换成 J。
 * 最终三段（按 10 条一段换前缀，段内回到数字）：
 *   · `Ctrl+B` + `1..9 0` → 文档第 1–10 条
 *   · `Ctrl+J` + `1..9 0` → 文档第 11–20 条
 *   · `Ctrl+M` + `1..5`    → 文档第 21–25 条
 * 现场记法变成"第几段用哪个前缀、段内第几个"，三段都是数字键，右手不用来回挪。
 *
 * ⚠ 选前缀时要避开浏览器自己的快捷键：`Ctrl+N`（新窗口）、`Ctrl+T`（新标签）、
 *   `Ctrl+W`（关标签）、`Ctrl+D`（收藏）、`Ctrl+L`（地址栏）—— 这些即使被
 *   `preventDefault` 拦下一部分，在某些浏览器/扩展下仍会漏。B / J / M 三个都没有默认动作。
 *
 * ⚠ 同一个数字键在三段里含义不同（`Ctrl+B+1` 是第 1 条，`Ctrl+J+1` 是第 11 条），
...
export type SequenceState = {
  /**
   * 已经按下的**段前缀**（"b" / "j" / "m"），正等后面的数字；null = 还没按。
   *
...
export const SCRIPT_SHORTCUT_GROUPS = [
  ...
  /* 第 11–20 条（前缀用 J 不用 N：Ctrl+N 是浏览器"新建窗口"） */
  { prefix: "j", keys: ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"] },
  /* 第 21–25 条 */
  { prefix: "m", keys: ["1", "2", "3", "4", "5"] },
] as const;

/** 三段前缀（"b" / "j" / "m"），判定"这一次按的是不是段前缀"用 */
 ...
 *
 * 形如 `"b:1"`、`"j:0"`、`"m:5"`；第 i 个 id 就是第 i 条对话的键位。
...
export function advanceSequence(event: KeyLike, state: SequenceState, nowMs: number): SequenceVerdict {
  ...
  if (SCRIPT_SEQUENCE_PREFIX_KEYS.includes(key)) {
    /* 只记"按了哪个段前缀"与时刻，不在这里 preventDefault —— 由调用方在 fire 或 armed 时决定，
       见下方说明：Ctrl+B/J/M 在部分浏览器里有默认行为，需要拦；但纯函数不该碰事件 */
  ...
  /*
    ...
    这里刻意不查"所有段的键的并集"：`Ctrl+B+5` 与 `Ctrl+J+5`、`Ctrl+M+5` 是不同的条目，
