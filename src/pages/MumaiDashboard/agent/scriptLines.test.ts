...
test("每轮台词必须含 §6 冻结数据里的关键数字/标识（防数字走样）", () => {
  ...
  */
  const required: [string, string[]][] = [
    ...
    ["③", ["核对范围", "巡检", "辅助诊断", "可追溯"]],
    /*
      ④ 用户 2026-09-18 定的口径：小木只回「收到，已启用同步备份。」，
      文档第 4 条后半句（平台服务可访问…）**由讲解人自己说**，
      所以那半句不在 main 行里，而在 `role: "host"` 那行（下面第 2 组断言单独核）。
    */
    ["④", ["收到", "已启用同步备份"]],
    ["⑤", ["已按工单地点建立天气查询"]],
...
/* ------------------------------------------------------------------ *
 * 1b. 讲解人自己说的那半句仍在剧本里（第 ④ 轮，用户 2026-09-18 口径）
 * ------------------------------------------------------------------ */

test("第 ④ 轮：文档后半句登记为讲解人台词（role:host），小木的播报里不含它", () => {
  const round = SCRIPT_ROUNDS.find((r) => r.roundNo === "④");
  assert.ok(round, "第 ④ 轮不存在");
  const main = round!.lines.find((l) => l.role === "main");
  const host = round!.lines.find((l) => l.role === "host");
  assert.ok(main, "第 ④ 轮没有 main 台词");
  assert.ok(host, "第 ④ 轮缺少讲解人台词（role:host）—— 文档第 4 条后半句不能就这样丢掉");

  /* 小木只念「收到，已启用同步备份。」；文档那半句不许混进小木的播报 */
  assert.equal(main!.text, "收到，已启用同步备份。");
  assert.ok(!main!.text.includes("平台服务可访问"), "讲解人那半句不能出现在小木的台词里");
  /* 文档原句逐字保留在 host 行（改了就是改写文档） */
  assert.ok(host!.text.startsWith("平台服务可访问，任务已建立。"), `host 台词不是文档原句：${host!.text}`);
  assert.ok(host!.text.includes("采集时间各自独立记录。"));

  /* 只有 main 会被播报：executor 取的就是 role === "main" */
  assert.equal(round!.lines.filter((l) => l.role === "main").length, 1, "一轮只能有一句主台词");
});
