...
test("真实 25 轮：凡声明揭示的组，都必须排得进某一拍（一个都不能落下）", () => {
  ...
  /*
    ⚠ 阈值跟着剧本走，不是拍脑袋定的：2026-09-23 把 ⑪ 的揭示目标从工单详情改到
    **三维场景的四柱构件条**（`reveal.target = "twin-components"`，见 script.ts 里那段注释
    ——剧本这一轮演在三维场景，页面该跟着走），于是工单详情揭示从 8 轮减到 7 轮。
    这条断言的本意是"有多轮在用工单详情揭示"（而不是那个具体数字），
    所以这里跟着改成 7，并且**下界只许再降不许乱升**：升上去意味着新增了揭示声明。
  */
  assert.ok(rounds.length >= 7, `应有多轮声明了工单详情揭示（实际 ${rounds.length} 轮）`);
