...
export default function ModuleDrawer({ name, onClose, onOpenOrder }: { name: string; onClose: () => void; onOpenOrder: () => void }) {
  ...
  return <aside className="module-drawer" aria-live="polite">
    <header><span>业务模块</span><button onClick={onClose} aria-label="关闭模块概览"><Icon name="close"/></button></header>
    ...
    <button className="button button--primary" onClick={name === "工单档案" ? onOpenOrder : onClose}>{copy.action}<Icon name="arrow"/></button>
  </aside>;
}
