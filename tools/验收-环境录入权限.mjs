/**
 * 环境录入/校验权限验收（用户口径 2026-09-17）
 *
 * 用户原话：「我，shi账号下，应该是有权限填写环境记录与配置校验录入数据的，
 * 你干脆给我shi账号权限拉满得了」。
 *
 * 检验五条（含**反向**对照，证明不是把权限对所有人放开）：
 ...
 *   ⑥ 反向：史的 `canAssign` 仍是 false（指派权是 PRD §6.2 明令不给架构师的那一条）。
 *
...
try {
  if (orderId) {
    ...
    check("沈（项目经理）能指派（这条权限没有放开给史）", assign.status === 200, `HTTP ${assign.status}`);
    ...
    check("⑥ 史的 canAssign 仍为 false（PRD §6.2：架构师不拿指派权）", caps.canAssign === false, JSON.stringify(caps.canAssign));
