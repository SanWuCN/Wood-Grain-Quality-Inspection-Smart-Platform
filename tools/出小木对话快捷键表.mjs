/**
 * 生成《小木对话 · 快捷键对应表 v2.0》—— 给演示人照读的那张纸。
 *
 * ── 三条来源，一条都不手抄 ────────────────────────────────────────
...
const OUT = "D:\\平台\\小木对话-快捷键对应表-v3.0.md";
...
w("> · 第 **11–20** 条 → **`Ctrl+N+1`** … **`Ctrl+N+0`**（N 段）");
w("> · 第 **21–25** 条 → **`Ctrl+M+1`** … **`Ctrl+M+5`**（M 段）");
w(`> 两次按键之间要在 **${SCRIPT_SEQUENCE_WINDOW_MS / 1000} 秒**内完成（先按住 Ctrl 按段前缀 B/N/M，再按数字）。`);
w(">");
...
w();
w("> ⑬（`Ctrl+N+3`）是**唯一**小木自己起头的一轮：它会弹出**预警窗**（红色描边 +");
