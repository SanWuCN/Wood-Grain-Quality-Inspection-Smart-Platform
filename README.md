...
顶栏固定显示四格**真实状态**：**平台**（浏览器 ↔ 共享服务的实时通道）、**智能车**（当前巡检任务状态）、**扫描仪**（手持终端链路：真机在线 / 延迟 Ns / 离线 Ns / 未接入）、**模型**（当前演示模型版本）。每格的数据来源都接真实来源，时间与出处放在悬停说明里；右上角「状态正常 N/4」由这四格现算，不写死。
...
部署说明：[SensorTag 接入与验收](docs/SensorTag接入与验收.md)、[树莓派屏幕串流](tools/capture-screen/README.md)、[手持终端接入与验收](docs/手持终端接入与验收.md)。真实设备需要单独安装与绑定，克隆仓库不会自动获得本机设备配置或登录密钥。

---

## 手持终端（树莓派 Pi 5）与硬件监看

入口：`#/hardware?tab=monitor`。这里的四块 —— **设备状态 / 设备读数 / 通道状态 / 采集接收** —— 直接读手持终端上报的真实数据（终端每 2 秒 `POST /api/devices/{id}/hardware`，页面按同样节奏取最后一份）：

- **来源逐条标出**：读数按终端口径分「实测 / 推算 / 估算」三档；种子兜底时整块挂「模拟采集」；
- **没有的就是没有**：本机没有电量计，终端不下发「电池电量」，页面也不补默认值（这一行直接不出现，并写明原因）；
- **边界写在页面上**：能力声明里写明毫米波响应序列恒为回放（预制样例包）、无 IMU、无电量计；`map` / `pose` 两路按终端原话显示「断开 + 原因」；
- **断流只影响该设备**：≤6 秒在线、6—15 秒延迟、>15 秒离线；离线时保留最后一份真机数据并标「离线」，不回退成种子；
- **平台原来没有的四块**：能力声明、终端遥测（1 秒一条，只留最新一份）、设备事件与命令回执（accepted 与 executed 分开显示）、低帧率设备预览（明确不是归档图像）。

平台侧同时实现了设备通道：握手与心跳、事件按 `messageId` 去重、下行命令（查询状态 / 请求上传 / 暂停采集等，三态回执）、现场调参回写。终端不用改代码，把 `platform_url` 指到平台主机、`device_token` 与平台侧 `MUMAI_DEVICE_TOKENS` 对齐即可。

对接与验收：[手持终端接入与验收](docs/手持终端接入与验收.md)；契约回归：`node --test tools/test-device-gateway.mjs`。
...
原有命令执行链路按 `docs/语音语义Agent技术方案.md` 实现，**意图目录 29 条 / 工具 20 条**。

`RAO` 已合入 `SHI` 的常驻语音唤醒更新：开启“常驻唤醒”后说“小木小木”，支持流式字幕、命令回执和自动恢复聆听，命令复用现有智能体执行链路。唤醒识别依赖单独部署的本地 Python/KWS/Whisper 语音服务；该服务与模型未包含在此仓库。本次同步验证了构建、静态检查和原有采集回归，未在本机复验真人唤醒。安装及接口契约见 [语音唤醒接入说明](docs/语音唤醒接入-v1.0.md)。
...
5. 雷达批次流程与车辆运动仍为演示/只读路径。**手持终端（树莓派）那一路已接真机**：设备数据、遥测、事件与命令回执都来自真实终端，下行命令（查询状态 / 请求上传 / 暂停采集等）走设备通道并有 accepted → executed 三态回执；但终端的**文件交付**（分片上传 + 批次清单，`POST /api/files/uploads`、`POST /api/batches`）平台侧尚未实现，所以「采集接收」的计数会是 0（批次本身已在终端封存）。
6. 手持端的毫米波响应序列恒为**回放**（预制样例包），设备上没有真实毫米波回波；本机也没有 IMU 与电量计，能力声明里写明了这三条边界，页面不代它补齐。
