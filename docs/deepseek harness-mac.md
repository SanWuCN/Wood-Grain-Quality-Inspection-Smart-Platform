...
改代码用上面的 `npx vite`（dev server，带 HMR）；**演示要给别人看时用生产构建**：

```bash
./start-lan.sh              # 没有 dist 就先构建，然后启动
./start-lan.sh --build      # 强制重新构建
MUMAI_PORT=8080 ./start-lan.sh
```

它会打印本机与局域网两个地址，局域网内其他设备（同一 Wi-Fi／网段）直接打开
局域网那条即可。默认端口 **4173**，与 dev server 的 5173 分开，两个可以同时跑。

> 首次进 `/twin`（数字孪生）要加载 31 MB 的高斯泼溅产物，页面有进度条；
> 另外那一页吃 GPU，机器被别的重负载占着时（比如浏览器开着大页面）
> 可能出现渲染不出来或很慢 —— 演示前把无关的大页面关掉。
